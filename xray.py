    ...
    def _apply_api(self):
        ...
        if os.path.isfile(XRAY_EXCLUSIVE_CONFIG_PATH):
            with open(XRAY_EXCLUSIVE_CONFIG_PATH, 'r') as config_file:
                ...
                exclusive_config = json.load(config_file)
                # Inbounds
                self["inbounds"] = exclusive_config.get("inbounds", []) + \
                    self.get("inbounds", [])
                # Outbounds
                self["outbounds"] = exclusive_config.get("outbounds", []) + \
                    self.get("outbounds", [])
                # Routing
                self["routing"]["rules"] = exclusive_config.get("routing", {}).get("rules", []) + \
                    self["routing"]["rules"]
                # Reverse
                if "reverse" in exclusive_config:
                    self["reverse"] = exclusive_config["reverse"]
